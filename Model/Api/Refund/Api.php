...
namespace Lyranetwork\Micuentaweb\Model\Api\Refund;
...
class Api {
     ...
     *
     * @param \Lyranetwork\Micuentaweb\Model\Api\Refund\Processor $refundProcessor
     ...
     * Refund money.
     *
     * @param \Lyranetwork\Micuentaweb\Model\Api\Refund\OrderInfo $orderInfo
     * @param float $amount
     */
    public function refund($orderInfo, $amount)
    {
        ...
        // Get currency.
        $currency = MicuentawebApi::findCurrencyByAlphaCode($orderInfo->getOrderCurrencyIsoCode());
    ...
    {
        /**
         * @var Lyranetwork\Micuentaweb\Model\Api\Rest\Api $client
         * */
    ...
    private function checkRestResult($response, $expectedStatuses = array())
    {
        ...
        if ($response['status'] !== 'SUCCESS') {
            ...
            throw new \Lyranetwork\Micuentaweb\Model\Api\Refund\WsException($errorMessage, $answer['errorCode']);
    ...
    private function refundFromOneTransaction($orderInfo, $amountInCents, $transaction, $currency)
    {
        ...
        $successStatuses = array_merge(
            MicuentawebApi::getSuccessStatuses(),
            MicuentawebApi::getPendingStatuses()
        );
