...
class Multi extends Micuentaweb
{
    protected $_code = \Lyranetwork\Micuentaweb\Helper\Data::METHOD_MULTI;
    protected $_formBlockType = \Lyranetwork\Micuentaweb\Block\Payment\Form\Multi::class;

    protected $_canRefund = false;
    protected $_canRefundInvoicePartial = false;

    /**
     * @var \Lyranetwork\Micuentaweb\Model\System\Config\Source\MultiPaymentCard
     */
     ...
     * @param \Lyranetwork\Micuentaweb\Model\System\Config\Source\MultiPaymentCard $multiCardPayment
    ...
    public function __construct(
        ...
        \Lyranetwork\Micuentaweb\Model\System\Config\Source\MultiPaymentCard $multiCardPayment,
    ...
    protected function setExtraFields($order)
    {
        ...
        $option = @unserialize($info->getAdditionalInformation(\Lyranetwork\Micuentaweb\Helper\Payment::MULTI_OPTION));

        $amount = $this->micuentawebRequest->get('amount');
        $first = ! empty($option['first']) ? round(($option['first'] / 100) * $amount) : null;
        $this->micuentawebRequest->setMultiPayment($amount, $first, $option['count'], $option['period']);
        $this->micuentawebRequest->set('contracts', ($option['contract']) ? 'CB=' . $option['contract'] : null);

        $this->dataHelper->log('Multiple payment configuration is ' . $this->micuentawebRequest->get('payment_config')
    ...
    public function assignData(\Magento\Framework\DataObject $data)
    {
        ...
        $option = $this->getOption($micuentawebData->getData('micuentaweb_multi_option'));

        $info->setCcType($micuentawebData->getData('micuentaweb_multi_cc_type'))
            ->setAdditionalInformation(\Lyranetwork\Micuentaweb\Helper\Payment::MULTI_OPTION, serialize($option));
