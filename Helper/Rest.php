...
class Rest
{
    ...
    public function convertRestResult($answer, $isTransaction = false)
    {
        ...
        $response['vads_currency'] = MicuentawebApi::getCurrencyNumCode($this->getProperty($transaction, 'currency'));
        ...
        if ($transactionDetails = $this->getProperty($transaction, 'transactionDetails')) {
            ...
            $effectiveCurrency = MicuentawebApi::getCurrencyNumCode($this->getProperty($transactionDetails, 'effectiveCurrency'));
    ...
    public function checkResult($response, $expectedStatuses = [])
    {
        ...
        if ($response['status'] !== 'SUCCESS') {
            ...
            throw new \Lyranetwork\Micuentaweb\Model\RestException($msg, $answer['errorCode']);
