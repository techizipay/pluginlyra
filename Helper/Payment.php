...
class Payment
{
    ...
    const CHOOZEO_OPTION = 'micuentaweb_choozeo_option';

    // Key to save choosen Oney option.
    const ONEY_OPTION = 'micuentaweb_oney_option';

    // Key to save choosen Full CB option.
    const FULLCB_OPTION = 'micuentaweb_fullcb_option';

    // Key to save choosen Franfinance option.
    const FRANFINANCE_OPTION = 'micuentaweb_franfinance_option';

    // Key to save choosen other payment means.
    const OTHER_OPTION = 'micuentaweb_other_option';

    // Key to save risk control results.
    const RISK_CONTROL = 'micuentaweb_risk_control';

    // Key to save risk assessment results.
    const RISK_ASSESSMENT = 'micuentaweb_risk_assessment';

    // Key to save payment results.
    const ALL_RESULTS = 'micuentaweb_all_results';

    // Key to save Rest Api error message.
    const REST_ERROR_MESSAGE = 'micuentaweb_rest_error';

    const TRANS_UUID = 'micuentaweb_trans_uuid';

    const BRAND_USER_CHOICE = 'micuentaweb_brand_user_choice';
    ...
    public function registerOrder(
        ...
        if ($method instanceof \Lyranetwork\Micuentaweb\Model\Method\Sepa)  {
    ...
    public function nextOrderState(
        ...
        if ($response->isToValidatePayment()) {
            $newStatus = 'micuentaweb_to_validate';
            ...
            if ($this->isSepa($response)) {
                ...
                $newStatus = 'micuentaweb_pending_transfer';
            } else {
    ...
    public function updatePaymentInfo(\Magento\Sales\Model\Order $order, \Lyranetwork\Micuentaweb\Model\Api\MicuentawebResponse $response)
    {
        ...
        $order->getPayment()
            ...
            ->setAdditionalInformation(\Lyranetwork\Micuentaweb\Helper\Payment::ALL_RESULTS, serialize($response->getAllResults()))
            ->setAdditionalInformation(\Lyranetwork\Micuentaweb\Helper\Payment::TRANS_UUID, $response->get('trans_uuid'));
        ...
        if ($response->get('brand_management')) {
            ...
            $order->getPayment()->setAdditionalInformation(\Lyranetwork\Micuentaweb\Helper\Payment::BRAND_USER_CHOICE, $userChoice);
        }
        ...
        if ($response->get('card_brand') === 'MULTI') { // Multi brand.
            ...
            $option = @unserialize($order->getPayment()->getAdditionalInformation(\Lyranetwork\Micuentaweb\Helper\Payment::MULTI_OPTION));
            ...
            if ($isFirstInstallment && (stripos($order->getPayment()->getMethod(), 'micuentaweb_multi') === 0)
                ...
                for ($i = 1; $i <= $count; $i++) {
                    ...
                    if (($rate = $response->get('change_rate')) && $response->get('effective_currency') &&
                        ...
                        // Effective amount.
                        $effectiveCurrency = MicuentawebApi::findCurrencyByNumCode($response->get('effective_currency'));
    ...
    public function saveIdentifier(
        ...
        if ($response->get('identifier') && (
            ...
            $customerData->setCustomAttribute('micuentaweb_identifier', $response->get('identifier'));

            // Mask card number and save it to customer entity.
            $customerData->setCustomAttribute('micuentaweb_masked_pan', $this->maskPan($response));

            try {
                $customer->updateData($customerData);

                $customerResource = $this->customerResourceFactory->create();
                $customerResource->saveAttribute($customer, 'micuentaweb_identifier');
                $customerResource->saveAttribute($customer, 'micuentaweb_masked_pan');
    ...
    public function saveSepaIdentifier(
        ...
        if ($response->get('identifier') && (
            ...
            $customerData->setCustomAttribute('micuentaweb_sepa_identifier', $response->get('identifier'));

            // Mask IBAN and save it to customer entity.
            $customerData->setCustomAttribute('micuentaweb_sepa_iban_bic', $this->maskPan($response));

            try {
                $customer->updateData($customerData);

                $customerResource = $this->customerResourceFactory->create();
                $customerResource->saveAttribute($customer, 'micuentaweb_sepa_identifier');
                $customerResource->saveAttribute($customer, 'micuentaweb_sepa_iban_bic');
     ...
     *
     * @param string $micuentawebType
     * @return string
     */
    public function convertTransactionType($micuentawebType)
    {
        $type = false;

        switch ($micuentawebType) {
