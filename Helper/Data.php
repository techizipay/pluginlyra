...
{
    const METHOD_STANDARD = 'micuentaweb_standard';
    const METHOD_MULTI = 'micuentaweb_multi';
    const METHOD_CHOOZEO = 'micuentaweb_choozeo';
    const METHOD_SEPA = 'micuentaweb_sepa';
    const METHOD_GIFT = 'micuentaweb_gift';
    const METHOD_ONEY = 'micuentaweb_oney';
    const METHOD_PAYPAL = 'micuentaweb_paypal';
    const METHOD_FULLCB = 'micuentaweb_fullcb';
    const METHOD_FRANFINANCE = 'micuentaweb_franfinance';
    const METHOD_OTHER = 'micuentaweb_other';
    ...
    /**
     * @var \Lyranetwork\Micuentaweb\Model\Logger\Micuentaweb
     */
    protected $logger;
    ...
    /**
     * @param \Lyranetwork\Micuentaweb\Helper\Context $context
     * @param \Magento\Store\Model\StoreManagerInterface $storeManager
     * @param \Magento\Framework\App\MaintenanceMode $maintenanceMode
     * @param \Magento\Config\Model\ResourceModel\Config $resourceConfig
     * @param \Magento\Framework\Filesystem $filesystem
     * @param \Lyranetwork\Micuentaweb\Model\Logger\Micuentaweb
    ...
    public function __construct(
        \Lyranetwork\Micuentaweb\Helper\Context $context,
        \Magento\Store\Model\StoreManagerInterface $storeManager,
        \Magento\Framework\App\MaintenanceMode $maintenanceMode,
        \Magento\Config\Model\ResourceModel\Config $resourceConfig,
        \Magento\Framework\Filesystem $filesystem,
        \Lyranetwork\Micuentaweb\Model\Logger\Micuentaweb $logger,
    ...
    public function getCommonConfigData($field, $storeId = null)
    {
        ...
        $value = $this->scopeConfig->getValue('micuentaweb/general/' . $field, $scope, $storeId);
    ...
    {
        $filePath = $this->filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath('micuentaweb/images/' . $fileName);
    ...
    {
        // $path is as payment_[lang]/micuentaweb/micuentaweb_[group]/micuentaweb_[sub_group]
        $parts = explode('/', $path);
    ...
    public function updateMultiPaymentModelConfig($options)
    {
        foreach ($options as $option) {
            $this->resourceConfig->saveConfig(
                'payment/micuentaweb_multi_' . $option['count'] . 'x/model',
                \Lyranetwork\Micuentaweb\Model\Method\Multix::class,
    ...
    public function getMultiPaymentModelConfig()
    {
        ...
        $select = $connection->select()
            ...
            ->where('path LIKE ?', 'payment/micuentaweb\_multi\_%x/model');
    ...
    public function updateOtherPaymentModelConfig($options)
    {
        foreach ($options as $option) {
            $this->resourceConfig->saveConfig(
                'payment/micuentaweb_other_' . $option['means'] . '/model',
                \Lyranetwork\Micuentaweb\Model\Method\Other::class,
                ScopeConfigInterface::SCOPE_TYPE_DEFAULT,
                0
            );
        }
    ...
    public function getOtherPaymentModelConfig()
    {
        ...
        $select = $connection->select()
            ->from($this->resourceConfig->getMainTable())
            ->where('path LIKE ?', 'payment/micuentaweb\_other\_%/model');
    ...
    public function getLogoImageSrc($name)
    {
        if ($this->isUploadFileImageExists($name)) {
            return $this->storeManager->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_MEDIA) .
                'micuentaweb/images/' . $name;
        } else {
            // Default logo from the installed plugin.
            $asset = $this->assetRepo->createAsset('Lyranetwork_Micuentaweb::images/' . $name);

            if ($this->isPublishFileImageExists($asset->getRelativeSourceFilePath())) {
                return $this->getViewFileUrl('Lyranetwork_Micuentaweb::images/' . $name);
            }
        }
    ...
    {
        $standardMethod = $this->getMethodInstance(\Lyranetwork\Micuentaweb\Helper\Data::METHOD_STANDARD);
        $sepaMethod = $this->getMethodInstance(\Lyranetwork\Micuentaweb\Helper\Data::METHOD_SEPA);
    ...
    public function getCardDataEntryMode($storeId = null)
    {
        ...
        $value = $this->scopeConfig->getValue('payment/micuentaweb_standard/card_info_mode', $scope, $storeId);
    ...
    public function getContribParam() {
        ...
        return $cmsParam . '/' . $cmsVersion . '/' . MicuentawebApi::shortPhpVersion();
    }
}
