...
define(
    ...
    function($, Component, url) {
        ...
        $(window).on('hashchange', function(e) {
            if ((window.location.hash === '#payment') && $('#micuentaweb_oney').length) {
                $('#micuentaweb_oney_message').hide();
                $('#payment_form_micuentaweb_oney').hide();
        ...
        $(document).ready(function() {
            setTimeout(function() {
                var message = '';
                if ($('#micuentaweb_oney').length) {
                    // Show Oney error message if any.
                    message = $.cookie('micuentaweb_oney_error'); // Get error message from cookie.

                    if (message) {
                        html = '<div id="micuentaweb_oney_message" style="margin-bottom: 25px; width: 100%; background: #fae5e5; color: #e02b27; padding: 12px 0px 12px 25px; font-size: 1.3rem;">'
                            + '<span>' + message + '</span></div>';

                        $('#micuentaweb_oney_message').html(html);
                        $('#payment_form_micuentaweb_oney').hide();
                    }
                }
        ...
        return Component.extend({
            ...
            getPaymentMeansUrl: function() {
                return url.build('micuentaweb/customer/index');
            }
        });
    }
);
