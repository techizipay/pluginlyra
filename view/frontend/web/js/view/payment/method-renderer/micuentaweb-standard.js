...
define(
    ...
    function(
        ...
        return Component.extend({
            defaults: {
                template: 'Lyranetwork_Micuentaweb/payment/micuentaweb-standard',
                micuentawebCcType:  window.checkoutConfig.payment.micuentaweb_standard.availableCcTypes ?
                    window.checkoutConfig.payment.micuentaweb_standard.availableCcTypes[0]['value'] : null,
            ...
            getData: function() {
                ...
                if (this.isOneClick()) {
                    data['additional_data']['micuentaweb_standard_use_identifier'] = this.micuentawebUseIdentifier();
                }

                if (this.getEntryMode() == 2) { // Payment means selection on merchant site.
                    data['additional_data']['micuentaweb_standard_cc_type'] = this.micuentawebCcType();
                }
            ...
            updatePaymentBlock: function(blockName) {
                $('.payment-method._active .payment-method-content .micuentaweb-identifier li.micuentaweb-block').hide();
                $('li.micuentaweb-standard-' + blockName + '-block').show();
            },
            ...
            refreshToken: function(withSubmit) {
                ...
                storage.post(
                    url.build('micuentaweb/payment_rest/token?form_key=' + $.mage.cookies.get('form_key'))
            ...
            initIframeEvents: function(elts) { // To be called after iframe loading.
                ...
                quote.paymentMethod.subscribe(function(method) {
                    if ((method.method === 'micuentaweb_standard') || !me.iframeDisplayed) {
                        return;
                    }

                    var iframe = $('.payment-method .payment-method-content iframe.micuentaweb-iframe');
                    if (iframe && iframe.length) {
                        var cancelUrl = me.getIframeLoaderUrl() + '?mode=cancel' + '&' + Date.now();
                        iframe.attr('src', cancelUrl);

                        $('.payment-method .payment-method-content .micuentaweb-iframe').hide();
                        $('.payment-method .payment-method-content .micuentaweb-form').show();
            ...
            afterPlaceOrder: function() {
                ...
                if (me.getEntryMode() == 3) { // Iframe mode.
                    ...
                    $('.payment-method._active .payment-method-content .micuentaweb-form').hide();
                    $('.payment-method._active .payment-method-content .micuentaweb-iframe').show();

                    var iframe = $('.payment-method._active .payment-method-content iframe.micuentaweb-iframe');
                    if (iframe && iframe.length) {
