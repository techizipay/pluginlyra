 ...
 */
class CategoryMapping extends \Lyranetwork\Micuentaweb\Block\Adminhtml\System\Config\Form\Field\FieldArray\ConfigFieldArray
{
    /**
     * @var \Lyranetwork\Micuentaweb\Model\System\Config\Source\CategoryFactory
     */
    protected $micuentawebCategoryFactory;
     ...
     * @param \Lyranetwork\Micuentaweb\Model\System\Config\Source\CategoryFactory $micuentawebCategoryFactory
     * @param \Magento\Catalog\Model\CategoryFactory $categoryFactory
     * @param array $data
     */
    public function __construct(
        \Magento\Backend\Block\Template\Context $context,
        \Lyranetwork\Micuentaweb\Model\System\Config\Source\CategoryFactory $micuentawebCategoryFactory,
        \Magento\Catalog\Model\CategoryFactory $categoryFactory,
        array $data = []
    ) {
        $this->micuentawebCategoryFactory = $micuentawebCategoryFactory;
    ...
    public function _prepareToRender()
    {
        ...
        $options = [];
        foreach ($this->micuentawebCategoryFactory->create()->toOptionArray(true) as $option) {
            $options[$option['value']] = $option['label'];
        }

        $this->addColumn(
            'micuentaweb_category',
            [
                'label' => __('Mi Cuenta Web category'),
                'style' => 'width: 200px;',
                'renderer' => $this->getListRenderer('_micuentawebCategory', $options)
            ]
        );
        ...
        if ($allCategories && is_array($allCategories) && ! empty($allCategories)) {
            foreach ($allCategories as $code => $name) {
                $value[uniqid('_' . $code . '_')] = [
                    ...
                    'micuentaweb_category' => 'FOOD_AND_GROCERY',
                    'mark' => true
                ];
            }
        }
