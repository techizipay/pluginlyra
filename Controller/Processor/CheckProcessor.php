...
class CheckProcessor
{
    ...
    public function prepareResponse($params)
    {
        ...
        if (! $response->isAuthentified()) {
            ...
            $this->dataHelper->log(
                "{$this->dataHelper->getIpAddress()} tries to access micuentaweb/payment/check page without valid signature with parameters: " . json_encode($params),
                \Psr\Log\LogLevel::ERROR
            );

            $this->dataHelper->log(
                'Signature algorithm selected in module settings must be the same as one selected in Mi Cuenta Web Back Office.',
                \Psr\Log\LogLevel::ERROR
            );
